...
def incoming_offers(db: Session = Depends(get_db), save_model=Depends(utils.get_current_save)):
    ...
    """
    result_show = []
    offer_list = crud.get_offers_by_target_club(
        db=db, save_id=save_model.id, target_club_id=save_model.player_club_id, season=save_model.season
    )
    for offer in offer_list:
        player_info = computed_data_app.ComputedPlayer(
            player_id=offer.target_id, db=db, season=save_model.season, date=save_model.date
        ).get_show_data()
...
def dealing_offers(
    offer_id: int, answer: str, db: Session = Depends(get_db), save_model=Depends(utils.get_current_save)
):
    """
    玩家处理报价
    """
    ...
    if answer == "yes":
        crud.delete_target_by_player_id_n_buyer_id(
            db=db, target_id=offer.target_id, buyer_id=offer.buyer_id
        )  # 从target表中删除
        ...
        user_finance = schemas.UserFinanceCreate(
            save_id=save_model.id, amount=offer.offer_price, event="球员卖出", date=date
        )
        crud.add_user_finance(db=db, user_finance=user_finance)
        p = transfer_app.Player(db=db, player_id=offer.target_id, season=save_model.season, date=save_model.date)
...
def make_offer_by_user(
    target_player_id: int, offer_price: int, db: Session = Depends(get_db), save_model=Depends(utils.get_current_save)
):
    negotiate_list = crud.get_offers_from_player_by_status(
        db=db, save_id=save_model.id, buyer_id=save_model.player_club_id, season=save_model.season, status="s"
    )
    for offer in negotiate_list:
        if offer.target_id == target_player_id:
            return {"status": "repeat offer"}
    negotiate_list = crud.get_offers_from_player_by_status(
        db=db, save_id=save_model.id, buyer_id=save_model.player_club_id, season=save_model.season, status="n"
    )
    for offer in negotiate_list:
        if offer.target_id == target_player_id:
            return {"status": "repeat offer"}
    negotiate_list = crud.get_offers_from_player_by_status(
        db=db, save_id=save_model.id, buyer_id=save_model.player_club_id, season=save_model.season, status="u"
    )
    for offer in negotiate_list:
        if offer.target_id == target_player_id:
            return {"status": "repeat offer"}
    user_transfer_club = transfer_app.Club(
        db=db, club_id=save_model.player_club_id, date=save_model.date, season=save_model.season
    )
    if user_transfer_club.team_model.finance < offer_price:
        return {"status": "can not afford"}
    user_transfer_club.make_offer_by_user(
        save_id=save_model.id, target_player_id=target_player_id, offer_price=offer_price
    )
    return {"status": "succeed"}
...
# 玩家与球员协商工资
def negotiate_wage(
    target_player_id: int, offer_wage: float, db: Session = Depends(get_db), save_model=Depends(utils.get_current_save)
):
    target_player = transfer_app.Player(
        db=db, player_id=target_player_id, season=save_model.season, date=save_model.date
    )
    player_status = target_player.negotiate_wage(
        offer_wage=offer_wage, save_id=save_model.id, buyer_club_id=save_model.player_club_id
    )
    if player_status == 1:
...
def get_on_sale_players(
    ...
    order：0升序，1降序
    """
    db_players: List[models.Player] = crud.get_on_sale_players_by_save(
        db=db, save_id=save_model.id, offset=offset, limit=limit, order=order, attri=attri
    )
    player_show: List[schemas.PlayerShow] = []
    for player_model in db_players:
        if player_model.club_id != save_model.player_club_id:
            player_info = computed_data_app.ComputedPlayer(
                player_id=player_model.id, db=db, season=save_model.season, date=save_model.date
            ).get_show_data()
...
def get_players_by_attri(
    ...
    """
    db_players: List[models.Player] = crud.get_all_players_by_save_n_attri(
        db=db, save_id=save_model.id, attri=attri, value=value, offset=offset, limit=limit
    )
    player_show: List[schemas.PlayerShow] = []
    if db_players:
        for player_model in db_players:
...
# 获取待谈判列表
def get_negotiate_list(db: Session = Depends(get_db), save_model=Depends(utils.get_current_save)):
    result_show = []
    negotiate_list = crud.get_offers_from_player_by_status(
        db=db, save_id=save_model.id, buyer_id=save_model.player_club_id, season=save_model.season, status="n"
    )
...
# 获取被拒绝列表
def get_rejected_offers(db: Session = Depends(get_db), save_model=Depends(utils.get_current_save)):
    result_show = []
    negotiate_list = crud.get_offers_from_player_by_status(
        db=db, save_id=save_model.id, buyer_id=save_model.player_club_id, season=save_model.season, status="r"
    )
    for offer in negotiate_list:
        player_info = computed_data_app.ComputedPlayer(
            player_id=offer.target_id, db=db, season=save_model.season, date=save_model.date
