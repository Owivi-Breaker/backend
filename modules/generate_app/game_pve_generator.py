...
class GamePvEGenerator:
    ...
    def create_game_pve(
        self, player_club_id: int, computer_club_id: int, game: dict, date: str, season: int
    ) -> models.game_pve:
        """
    ...
    def create_team_n_player_pve(self) -> int:
        ...
        player_team_pve: schemas.TeamPvECreate = schemas.TeamPvECreate(
            **{"club_id": player_club_id, "created_time": datetime.datetime.now(), "is_player": True}
        )
        ...
        computer_team_pve: schemas.TeamPvECreate = schemas.TeamPvECreate(
            **{"club_id": computer_club_id, "created_time": datetime.datetime.now()}
        )
        ...
        player_selector = PlayerSelector(
            club_id=computer_club_id, db=self.db, season=self.save_model.season, date=self.save_model.date
        )
        ...
        tactic_adjustor = game_app.TacticAdjustor(
            db=self.db,
            club1_id=player_club_id,
            club2_id=computer_club_id,
