...
class NextTurner:
    ...
    def play_game(self, calendar_game):
        ...
        club1_model = self.db.query(models.Club).filter(models.Club.id == clubs_id[0]).first()
        club2_model = self.db.query(models.Club).filter(models.Club.id == clubs_id[1]).first()

        tactic_adjustor = game_app.TacticAdjustor(
            db=self.db,
            club1_id=clubs_id[0],
            club2_id=clubs_id[1],
            player_club_id=self.save_model.player_club_id,
            save_id=self.save_model.id,
            club1_model=club1_model,
            club2_model=club2_model,
        ...
        logger.info(
            "{} {}: {} {}:{} {}".format(
                calendar_game["game_name"], calendar_game["game_type"], name1, score1, score2, name2
            )
        )
    ...
    def pve_starter(self, pve: list):
        ...
        if self.skip:
            ...
            game_pve_generator = generate_app.GamePvEGenerator(db=self.db, save_model=self.save_model)
    ...
    def offer_expire_starter(self, offer_expire: list):
        ...
        target_offers = crud.get_offers_from_player_by_status(
            db=self.db,
    ...
    def transfer_starter(self, transfer: list):
        ...
        for club in clubs:
            if club.id != self.save_model.player_club_id:
                transfer_club = transfer_app.Club(
                    db=self.db, club_id=club.id, date=self.save_model.date, season=self.save_model.season
                )
                transfer_club_list.append(transfer_club)
        for transfer_club in transfer_club_list:
    ...
    def rank_n_tv_income_base(
        self, league: models.League, game: computed_data_app.computed_game, first_bonus, second_bonus, rest_bonus
    ):
        ...
        if first_club.id == self.save_model.player_club_id:
            user_finance = schemas.UserFinanceCreate(save_id=self.save_id, amount=first_bonus, event="转播收益", date=date)
            crud.add_user_finance(db=self.db, user_finance=user_finance)
        first_club.finance += 2500  # 联赛排名
        if first_club.id == self.save_model.player_club_id:
            user_finance = schemas.UserFinanceCreate(save_id=self.save_id, amount=2500, event="联赛排名收益", date=date)
            crud.add_user_finance(db=self.db, user_finance=user_finance)
        ...
        if second.id == self.save_model.player_club_id:
            user_finance = schemas.UserFinanceCreate(save_id=self.save_id, amount=second_bonus, event="转播收益", date=date)
            crud.add_user_finance(db=self.db, user_finance=user_finance)
        second.finance += 1500
        if second.id == self.save_model.player_club_id:
            user_finance = schemas.UserFinanceCreate(save_id=self.save_id, amount=1500, event="联赛排名收益", date=date)
            crud.add_user_finance(db=self.db, user_finance=user_finance)
        ...
        for i in range(3, len(point_table)):
            ...
            if club.id == self.save_model.player_club_id:
                user_finance = schemas.UserFinanceCreate(
                    save_id=self.save_id, amount=rest_bonus, event="转播收益", date=date
                )
                crud.add_user_finance(db=self.db, user_finance=user_finance)
    ...
    def rank_n_tv_income(self):
        ...
        for league in leagues:
            if league.name == "英超":
                self.rank_n_tv_income_base(
                    league=league, game=game, first_bonus=15000, second_bonus=9000, rest_bonus=4800
                )
            elif league.name == "西甲":
                self.rank_n_tv_income_base(
                    league=league, game=game, first_bonus=12500, second_bonus=7500, rest_bonus=4000
                )
            elif league.name == "德甲":
                self.rank_n_tv_income_base(
                    league=league, game=game, first_bonus=10200, second_bonus=6200, rest_bonus=3550
                )
            elif league.name == "意甲":
                self.rank_n_tv_income_base(
                    league=league, game=game, first_bonus=10000, second_bonus=6000, rest_bonus=3200
                )
            elif league.name == "法甲":
                self.rank_n_tv_income_base(
                    league=league, game=game, first_bonus=8750, second_bonus=5250, rest_bonus=2800
                )
            elif league.name == "英冠":
                self.rank_n_tv_income_base(
                    league=league, game=game, first_bonus=6250, second_bonus=3750, rest_bonus=2000
                )
            elif league.name == "西乙":
                self.rank_n_tv_income_base(
                    league=league, game=game, first_bonus=5625, second_bonus=3375, rest_bonus=1800
                )
            else:
                self.rank_n_tv_income_base(
                    league=league, game=game, first_bonus=5000, second_bonus=3000, rest_bonus=1600
                )
        ...
        for league in leagues:
            for club in league.clubs:
                ...
                if club.id == self.save_model.player_club_id:
                    ...
                    date = datetime.datetime(int(year), int(month), int(day))
                    user_finance = schemas.UserFinanceCreate(
                        save_id=self.save_id, amount=club.reputation**3 * 0.02 + 500, event="广告收益", date=date
                    )
                    crud.add_user_finance(db=self.db, user_finance=user_finance)
        ...
        for league in leagues:
            for club in league.clubs:
                ...
                if club.id == self.save_model.player_club_id:
                    ...
                    date = datetime.datetime(int(year), int(month), int(day))
                    user_finance = schemas.UserFinanceCreate(
                        save_id=self.save_id, amount=-sum_salary, event="球员与职员工资", date=date
                    )
                    crud.add_user_finance(db=self.db, user_finance=user_finance)
